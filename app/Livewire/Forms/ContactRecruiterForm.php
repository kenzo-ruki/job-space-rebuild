...
class ContactRecruiterForm extends Component
{
    use WithFileUploads;
     ...
     */
    public function mount(Application $resume)
    {
    ...
    {
        return view('forms.contact-recruiter-form');
    }
    ...
    {
        $resume = Application::where('id', '=', $this->resume_id)->get()->first();
        $job = Job::where('job_id', '=', $resume->job_id)->get()->first();
        try {
            ...
            Mail::to('seth@jobspace.co.nz')
                ->send(new ContactRecruiter($job, $resume, $attachmentPath));
                $message = new ApplicantInteraction([
                    'jobseeker_id' => auth()->user()->jobseeker_id,
                    'recruiter_id' => $resume->recruiter_id,
