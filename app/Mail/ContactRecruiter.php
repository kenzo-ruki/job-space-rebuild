...
class ContactRecruiter extends Mailable
{
    ...
    public function __construct(Job $job, Application $application, $attachment = '')
    {
        ...
        $this->introLines = "<p>Hello {$this->recruiter->recruiter_first_name} {$this->recruiter->recruiter_last_name}</p><p>{$this->applicant->first_name} {$this->applicant->last_name} has replied to your message regarding the job {$job->job_title} (Reference Number: {$job->job_reference}). Please log in to your Jobspace account to check the message.</p>";
