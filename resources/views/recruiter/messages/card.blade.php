@php
$message = App\Models\ApplicantInteraction::find($message->id);
$application = App\Models\Application::where('id', $message->application_id)->first();
$job = null;
$recruiter = null;
$jobseeker = null;
...
} else if ($message->sender === 'jobseeker' && $message?->jobseeker_id) {
    $jobseeker = App\Models\User::where('jobseeker_id', $message->jobseeker_id)->first();
} else if ($application) {
    $job = App\Models\Job::where('job_id', $application->job_id)->first();
    if (!empty($job) && empty($recruiter)) {
        $recruiter = App\Models\Recruiter::where('recruiter_id', $job->recruiter_id)->first();
    }
    if (!empty($job) && empty($jobseeker)) {
        $jobseeker = App\Models\User::where('jobseeker_id', $application->jobseeker_id)->first();
    }
}
@endphp
<tr>
        ...
        <div class="p-3">
            <div class="hs-dropdown relative inline-block [--placement:bottom-right]">
                ...
                <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-40 z-10 bg-white shadow-md rounded-2xl p-4 mt-2" aria-labelledby="dropdown-{{$message->id}}">
                    <a href="{{ route('recruiter.message.view', ['message' => $message->id]) }}" class="py-2 px-3 mb-4 flex block items-center gap-x-1 text-xs font-medium border border-blue-violet-800 bg-blue-violet-100 text-blue-violet-800 rounded-full">
                    ...
                    <a href="{{ route('recruiter.message.reply', ['message' => $message->id]) }}" class="py-2 px-3 mb-4 flex block items-center gap-x-1 text-xs font-medium border border-teal-800 bg-teal-100 text-teal-800 rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
