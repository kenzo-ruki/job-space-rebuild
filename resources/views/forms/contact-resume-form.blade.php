...
<form wire:submit="save">
    ...
    <div class="flex flex-col sm:flex-row mb-6 sm:mb-12">
        <input type="text" required wire:model="contact_resume_subject" name="contact_resume_subject" id="contact_resume_subject" placeholder="Subject*" class="rounded-full w-full h-12 px-6 pr-16 text-sm text-slate-600 placeholder-slate-400 border border-slate-300 focus:outline-none focus:ring-1 focus:ring-slate-400 focus:border-transparent">
    </div>   
    @error('contact_resume_subject') <div class="w-full text-cerise-red-500 mt-2 text-left pl-6"><span class="error">{{ $message }}</span></div> @enderror

    <div class="flex flex-col sm:flex-row mb-6 sm:mb-12">
        
        <div class="flex-grow" wire:ignore>
            <textarea wire:model="contact_resume_message"
                wire:model.debounce.999999ms="contact_resume_message"
                wire:key="contact_resume_message"
                ...
                name="contact_resume_message"
                id="contact_resume_message"></textarea>
        </div>
        @push('tinymce')
            @include('components.tinymce.instance', ['selector' => '#contact_resume_message', 'wireModel' => 'contact_resume_message', 'wordCount' => false])
        @endpush
        @error('contact_resume_message') <div class="w-full text-cerise-red-500 mt-2 text-left pl-6"><span class="error">{{ $message }}</span></div> @enderror
    </div>
    ...
    <div class="flex flex-col sm:flex-row mb-6 sm:mb-12">
        <div class="mx-auto inline-flex items-center rounded-full font-bold h-14 px-8 text-white border-1-cerise-red-500 bg-cerise-red-500 hover:bg-cerise-red-600 transition-bg duration-[600ms]">
            ...
            <button type="submit" class="">Contact Resume Holder</button>
        </div>
    </div>
</form>
<!-- End Card -->