<div class="overflow-x-auto min-w-full divide-y divide-gray-200">
    <table class="min-w-full">
        ...
        @php
        if ($job->expired < Carbon\Carbon::now()) {
            continue;
        }
        $jobLocation = getJobLocation($job);
        $jobCategories = getJobCategory($job);
        @endphp
            <tr>
                <td class="size-px text-center text-xs font-bold">
                    <div class="p-3">
                        <a href="/jobs/{{$job->job_id}}" target="_blank" class="text-bold text-cerise-red-600">{{ $job->job_title }}</a>
                    </div>
                </td>
                ...
                <td class="size-px text-center text-xs whitespace-nowrap">
                    <div class="p-3">
                        <div class="hs-dropdown relative inline-block [--placement:bottom-right]">
                            ...
                            <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-40 z-10 bg-white shadow-md rounded-2xl p-4 mt-2" aria-labelledby="dropdown-saved-{{$job->job_id}}">
                                <a href="{{ route('apply.create', $job->job_id) }}" class="py-2 px-3 flex block mb-4 items-center gap-x-1 text-xs font-medium border border-blue-violet-800 bg-blue-violet-100 text-blue-violet-800 rounded-full">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.59 14.37a6 6 0 0 1-5.84 7.38v-4.8m5.84-2.58a14.98 14.98 0 0 0 6.16-12.12A14.98 14.98 0 0 0 9.631 8.41m5.96 5.96a14.926 14.926 0 0 1-5.841 2.58m-.119-8.54a6 6 0 0 0-7.381 5.84h4.8m2.581-5.84a14.927 14.927 0 0 0-2.58 5.84m2.699 2.7c-.103.021-.207.041-.311.06a15.09 15.09 0 0 1-2.448-2.448 14.9 14.9 0 0 1 .06-.312m-2.24 2.39a4.493 4.493 0 0 0-1.757 4.306 4.493 4.493 0 0 0 4.306-1.758M16.5 9a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Z" />
                                    </svg>
                                    Apply
                                </a>
                                <a href="{{ route('jobs.single_id', $job->job_id) }}" class="py-2 px-3 flex block mb-4 items-center gap-x-1 text-xs font-medium border border-teal-800 bg-teal-100 text-teal-800 rounded-full">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m12.75 15 3-3m0 0-3-3m3 3h-7.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                    </svg>
                                    View
                                </a>
                                <form method="POST" action="{{ route('saved-jobs.destroy', $job->job_id) }}" class="w-full">
                                    @csrf
                                    @method('DELETE')
