                    ...
                    <td class="size-px text-center text-xs whitespace-nowrap">
                        <div class="p-3">
                            <div class="hs-dropdown relative inline-block [--placement:bottom-right]">
                                ...
                                <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-40 z-10 bg-white shadow-md rounded-2xl p-4 mt-2" aria-labelledby="dropdown-{{$jobAlert->id}}">
                                    <a href="{{ route('jobalert.jobseeker.edit', $jobAlert->id) }}" class="py-2 px-3 flex block mb-4 items-center gap-x-1 text-xs font-medium border border-teal-800 bg-teal-100 text-teal-800 rounded-full">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="m12.75 15 3-3m0 0-3-3m3 3h-7.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                        </svg>
                                        Edit
                                    </a>
                                    <form method="POST" action="{{ route('jobalert.jobseeker.destroy', $jobAlert->id) }}" class="w-full">
                                        @csrf
                                        @method('DELETE')
