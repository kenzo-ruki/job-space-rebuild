@php 
$recruiter = false;
if ($message->recruiter_id !== 0) {
    $recruiter = App\Models\Recruiter::find($message->recruiter_id);
}
$jobseeker = auth()->user();
@endphp
<tr>
    ...
    <td class="size-px text-center text-xs px-6 py-4 text-sm text-slate-500 hidden md:table-cell">
        <div class="p-3">
            @if(($message->sender === 'recruiter' || $message->jobseeker_id === 0) && $recruiter ) From: {{ $recruiter->recruiter_first_name }} {{ $recruiter->recruiter_last_name }} @else To: @endif {{ $jobseeker->first_name }} {{ $jobseeker->last_name }}
        </div>
    </td>
    <td class="size-px text-center text-xs px-6 py-4 text-sm text-slate-500 hidden md:table-cell">
        <div class="p-3">
        ...
        <div class="p-3">
            <div class="hs-dropdown relative inline-block [--placement:bottom-right]">
                ...
                <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-40 z-10 bg-white shadow-md rounded-2xl p-4 mt-2" aria-labelledby="dropdown-{{$message->id}}">
                    <a href="{{ route('jobseeker.message.view', ['message' => $message->id]) }}" class="py-2 px-3 mb-4 flex block items-center gap-x-1 text-xs font-medium border border-blue-violet-800 bg-blue-violet-100 text-blue-violet-800 rounded-full">
                    ...
                    <a href="{{ route('jobseeker.message.reply', ['message' => $message->id]) }}" class="py-2 px-3 mb-4 flex block items-center gap-x-1 text-xs font-medium border border-teal-800 bg-teal-100 text-teal-800 rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
